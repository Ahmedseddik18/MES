...
$selectFields = 'db.commande, db.article,  commande.sub, db.phase, db.dateChargement';
$fromTables = 'db LEFT JOIN commande ON db.commande = commande.commande';
$whereConditions = "commande.TSC = '' or commande.TSC is null";
$groupBy = 'GROUP BY db.commande, db.article,  commande.sub';
$orderBy = 'ORDER BY db.commande, db.article,  commande.sub '; // Trier par ces colonnes

// Appeler la fonction pour obtenir les résultats groupés
$result = getGroupedResults($conn, $selectFields, $fromTables, $whereConditions, $groupBy, $orderBy);
        ...
        if ($result && $result->rowCount() > 0) {
            ...
            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo "<a href='ajouter-temps-standard.php?commande=" . htmlspecialchars($row['commande']) . "&article=" . htmlspecialchars($row['article']) . "&sub=" . htmlspecialchars($row['sub']) . "'><i class='bx bxs-pencil'></i></a>"; // Icône d'édition
                echo "</td>";	
