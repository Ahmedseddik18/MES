...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $selectedOrders = isset($_POST['selectedOrders']) ? $_POST['selectedOrders'] : [];
        
        // Récupérer les opérateurs sélectionnés
        $selectedOperators = isset($_POST['selectedOperators']) ? $_POST['selectedOperators'] : [];
...
// Initialiser des variables pour le suivi des succès et des erreurs
$allSuccess = true; // Pour vérifier si toutes les opérations réussissent
$message = '';
...
foreach ($plan_de_travail as $ligne) {
    ...
    foreach (['manutention', 'matelasseur', 'coupeur', 'etiquetage'] as $phase) {
        ...
        if (isset($ligne[$phase . 'operateur1'])) {
            ...
            // Modifier la phase selon le besoin
            ...
            if (!$stmt->execute()) {
                // S'il y a une erreur, mettre à jour le message et le statut
                $allSuccess = false;
                $errorInfo = $stmt->errorInfo();
                $message = 'Erreur lors de la mise à jour pour ' . $ligne['commande'] . ' - ' . $ligne['article'] . ': ' . $errorInfo[2];
            }
        }
    }
}

// Déterminer le message final basé sur le statut
if ($allSuccess) {
    $message = 'Toutes les mises à jour ont été effectuées avec succès.';
} else {
    // Si au moins une mise à jour a échoué, garder le message d'erreur
    $message = 'Certaines mises à jour ont échoué : ' . $message;
}

// Retourner la réponse JSON
header('Content-Type: application/json');
echo json_encode(['success' => $allSuccess, 'message' => $message]);
