...
$quantiteDemandee = $_GET['quantitedemandee'];
$formData = [
    'commande' => '',
    'article' => '',
    'phase' => '',
...
if (isset($_POST['Suivant'])) {
    ...
    foreach ($formData as $key => $value) {
        if (empty($value) && !in_array($key, ['nbSub', 'semaine', 'traitement', 'commande', 'article', 'quantiteDemandee', 'gestion'])) {
    ...
    if (empty($message)) {
        ...
        if ($uploadResult['status'] === 'success') {
            ...
            // Déclaration des données à mettre à jour
            $data = [
                ...
                'etat' => $etat,
                'fichier' => $uploadedFilesStr,
                'subvalues' => $formData['subValues']
            ];

            // Condition WHERE pour sélectionner la bonne commande et l'article
            $whereConditions = "commande = :commande AND article = :article";
            $params = ['commande' => $commande, 'article' => $article];

            // Appel de la fonction `updateData` pour mettre à jour les données
            $result = updateData('db', $data, $whereConditions, $params, $conn);
...
La commande <strong>$commande</strong> pour l'article <strong>$article</strong> a été chargée au sein du coupe interne.<br><br>
...
<td>{$formData['dateChargement']}</td>
    ...
    <td>{$formData['gestion']}</td>
    <td>{$formData['variante']}</td>
</tr>";
...
sendEmail($testRecipients, $subject, $message , $cc);
                    header("Location: ajouter-details-matelas.php?nbPartie={$formData['nbPartie']}&subValues={$formData['subValues']}&commande=$commande&article=$article&phase={$formData['phase']}&dateChargement={$formData['dateChargement']}");
                    exit();
                }
            } else {
...
<script>
    function format(input) {
        let value = input.value.toUpperCase(); // Convertir en majuscules
        value = value.replace(/[^A-Z0-9]/g, ''); // Supprimer les caractères non autorisés
        let formattedValue = '';

        // Ajouter des virgules après chaque groupe de 3 caractères
        for (let i = 0; i < value.length; i += 3) {
            if (i > 0) {
                formattedValue += ','; // Ajouter une virgule avant chaque groupe sauf le premier
            }
            formattedValue += value.slice(i, i + 3); // Ajouter le groupe de 3 caractères
        }

        input.value = formattedValue; // Mettre à jour la valeur de l'input
    }
</script>
