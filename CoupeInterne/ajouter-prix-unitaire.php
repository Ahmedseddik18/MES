...
$selectFields = 'commande, article, phase, categorie, traitement, relaxation, tissu, quantitedemandee, nbpartie, nbsub';
$fromTables = 'db';
...
if (isset($_POST['Suivant'])) {
    ...
    // Vérifier si le prix unitaire est non vide
    if (!empty($prixUnitaire)) {
        // Créer un tableau associatif pour les données à mettre à jour
        $data = array('prixunitaire' => $prixUnitaire);

        // Définir la table et les conditions WHERE
        $fromTables = 'db'; // Remplacez 'votre_table' par le nom réel de la table
        $whereConditions = 'id = :id';
        
        // Créer un tableau de paramètres pour les valeurs WHERE
        $params = [
            'id' => $id
        ];

        // Mettre à jour les données en utilisant la fonction `updateData`
        $result = updateData($fromTables, $data, $whereConditions, $params, $conn);

        // Assigner les valeurs d'alerte
        $alertClass = $result['alertClass'];
        $message = $result['message'];

        // Vérifier le succès de l'opération
        if ($alertClass === 'alert-success') {
            // Redirection vers la page ajouter-temps-standard2.php avec les paramètres
            $commande = $row['commande'];
            $article = $row['article'];
            header("Location: modifier-commandes.php");
            exit(); // Toujours appeler exit() après une redirection pour stopper l'exécution du script
        }
    } else {
        // Gérer le cas où prixUnitaire est vide
        $alertClass = 'alert-danger';
        $message = 'Le prix unitaire ne peut pas être vide.';
    }
}
  ...
  <body>
            ...
            <div class="flex-grow-1 container-p-y container-fluid">
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="card mb-4">
                    <h5 class="card-header">Cout Unitaire</h5>
    ...
    <div class="row">
        ...
        <div class="mb-3 col-md-6">
            ...
            <select id="categorie" name="categorie" class="select2 form-select" disabled >
                <option value=""></option>
            ...
            <div class="row gy-3">
                <div class="col-md">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="traitement[]" value="SM" id="SM" <?php echo in_array('SM', explode(',', $traitement)) ? 'checked' : ''; ?> disabled />
                        <label class="form-check-label" for="SM"> SM </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="traitement[]" value="RM" id="RM" <?php echo in_array('RM', explode(',', $traitement)) ? 'checked' : ''; ?> disabled />
        ...
        <div class="mb-3 col-md-6">
            ...
            <div class="row gy-3">
                <div class="col-md">
                    <div class="form-check">
                        <input name="relaxation" class="form-check-input" type="radio" value="Oui" id="RadioOUI" <?php echo $relaxation === 'Oui' ? 'checked' : ''; ?>  disabled />
                        <label class="form-check-label" for="RadioOUI"> Oui </label>
                    </div>
                    <div class="form-check">
                        <input name="relaxation" class="form-check-input" type="radio" value="Non" id="RadioNon" <?php echo $relaxation === 'Non' ? 'checked' : ''; ?> disabled />
        ...
        <div class="mb-3 col-md-6">
            ...
            <input class="form-control" type="text" id="tissu" name="tissu" maxlength="3" style="text-transform: uppercase;" disabled title="Veuillez entrer seulement les 3 lettres majuscules de code tissu" value="<?php echo $tissu; ?>" />
        </div>
        ...
        <div class="mb-3 col-md-6">
            ...
            <input
                ...
                id="quantite"
                name="quantite"
                value="<?php echo $quantiteDemandee; ?>"
				disabled
            />
        </div>
        ...
        <input
                    ...
                    id="nbSub"
                    name="nbSub"
                    value="<?php echo $nbSub; ?>"
					disabled
                />
        </div>
		<div class="mb-3 col-md-6">
    <label for="language" class="form-label">Cout Unitaire</label>
    <input
        type="text"
        class="form-control"
        id="prixUnitaire"
    ...
    />
		...
		<div class="mb-3 col-md-6">
            <label for="language" class="form-label">Cout Total</label>
        <input
                    type="number"
                    class="form-control"
                    id="prixTotal"
