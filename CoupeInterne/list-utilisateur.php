...
>
  <head>
    ...
    <title>Coupe Interne | Liste Des Utilisateurs</title>
...
<?php include 'navbar.php'; ?>
          ...
          <div class="content-wrapper">
            ...
            <div class="flex-grow-1 container-p-y container-fluid">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Authentifications /</span> Liste Utilisateurs</h4>
                ...
                <table class="table table-hover">
                    <thead class="text-dark fs-4">
                        <tr>
							...
							<th class="border-bottom-0">
                                <h6 class="fw-semibold mb-0">Privilège</h6>
                            </th>
                            <th class="border-bottom-0">
...
if ($result) {            // Utilisation de fetch() pour récupérer les résultats avec PDO
            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                ...
                if (htmlspecialchars($row['role']) == 'admin') {
                    echo "<td class='border-bottom-0'><span class='badge bg-label-primary me-1'>" . htmlspecialchars($row['role']) . "</span></td>";
                } elseif (htmlspecialchars($row['role']) == 'responsable-utm') {
                    echo "<td class='border-bottom-0'><span class='badge bg-label-success me-1'>" . htmlspecialchars($row['role']) . "</span></td>";
                } elseif (htmlspecialchars($row['role']) == 'responsable-coupe') {
                    echo "<td class='border-bottom-0'><span class='badge bg-label-warning me-1'>" . htmlspecialchars($row['role']) . "</span></td>";
                }else{
					echo "<td class='border-bottom-0'><span class='badge bg-label-info me-1'>" . htmlspecialchars($row['role']) . "</span></td>";
				}
                
				
                echo "<td class='border-bottom-0'>";
                echo "<a href='modifier-utilisateur.php?id=" . htmlspecialchars($row['id']) . "'><i class='bx bxs-pencil'></i></a>"; // Icône d'édition
