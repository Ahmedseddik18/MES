...
<div class="card ">
<h5 class="card-header">Archive Des Prix</h5>
    <div class="card-body">
        <?php
...
$article = isset($_GET['article']) ? strtoupper($_GET['article']) : '';
$phase = isset($_GET['phase']) ? strtoupper($_GET['phase']) : '';
$categorie = isset($_GET['categorie']) ? strtoupper($_GET['categorie']) : '';
$tissu = isset($_GET['tissu']) ? strtoupper($_GET['tissu']) : '';
...
$result = select($conn, $selectFields, $fromTables, $whereConditions, $orderBy);
    ...
    echo "<td class='border-bottom-0'>" . strtoupper(htmlspecialchars($row['prixunitaire'])) . " €</td>";
    ...
    echo "<a href='modifier-prix-unitaire.php?id=" . urlencode($row['id']) . "'>";
